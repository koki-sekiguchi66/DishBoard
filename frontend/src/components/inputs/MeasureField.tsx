 ...
 *   4. step 固定の ± / ドラッグでは、0.1g 単位のような細かい調整ができない
 *   5. 全角入力や一部端末のキーボードが出す「，」を小数点として扱えず、
 *      小数点以下がまったく入力できない環境がある
 * 1〜3 は、値を文字列で保持して空欄を許すこと・目盛りのドラッグと ± の長押し・
 * type="text" + inputMode="decimal" で解消している。
 * 4 は精密モード（isPrecise で step を1/10にする）、5 は sanitize() での
 * 全角/カンマ正規化で解消している（→ docs-public/decisions.md #18, #20）。
 *
