...
export default function MeasureField({
  ...
  const commit = useCallback(
    (next: number) => {
      const decimals = Math.min(
        MAX_DECIMALS,
        Math.max(decimalsOf(step), decimalsOf(valueRef.current))
      );
  ...
  const nudge = useCallback(
    (direction: 1 | -1, multiplier = 1) => {
      commit(toNumber(valueRef.current) + direction * step * multiplier);
    },
    [commit, step]
  );
  ...
  const handleDragMove = (e: ReactPointerEvent<HTMLDivElement>) => {
    ...
    commit(origin.value + Math.round(dx / PX_PER_TICK) * step);
  };
  ...
  const rulerOffset = -(toNumber(value) / step) * PX_PER_TICK;
  const edgeFade =
